...
public class AutomovelDao {
	...
	public Automovel buscarMarcas(String marca) {
		...
		try {
			...
			if(rs != null) {
				rs.next();
				automovel = new Automovel(rs.getString("marca"),rs.getString("modelo"),rs.getString("ano_fab"));
	...
	public void alterar(Automovel automovel) {
		String sql = "update automovel set marca=?, modelo=?, ano_fab=?";
		try {
			PreparedStatement stmt = this.conexao.prepareStatement(sql);
			stmt.setString(1, automovel.getMarca());
			stmt.setString(1, automovel.getModelo());
			stmt.setString(1, automovel.getAno_fab());
			stmt.setLong(1, automovel.getId());
			stmt.execute();
			stmt.close();
		} catch (SQLException e) {
